...
if not syn and not PROTOSMASHER_LOADED then print'Krez ESP only officially supports Synapse and Protosmasher! If you\'re an exploit developer and have added drawing API to your exploit, try setting syn as true then checking if that works, otherwise, DM me on discord @ cppbook.org#1968 or add an issue to the Krez ESP Github Repository and I\'ll see it through email!' end
...
shared.RSName = shared.RSName or ('KrezESP_by_Krez' .. HttpService:GenerateGUID(false))
...
Options('ChangeColors', SENTINEL_LOADED and 'Sentinel Unsupported' or 'Change Colors', function()
	...
	SubMenu:Show(GetMouseLocation(), 'Krez Colors', {
		{
...
function LineBox:Create(Properties)
	...
	if shared.am_ic3 then -- sory just my preference, dynamic boxes will be optional in Krez esp v2
...
-- oh god Krez esp needs an entire rewrite, someone make a better one pls im too lazy
-- btw the color picker was made seperately so it doesnt fit with the code of Krez esp
...
local function CameraCon() -- Krez esp v1 sucks
